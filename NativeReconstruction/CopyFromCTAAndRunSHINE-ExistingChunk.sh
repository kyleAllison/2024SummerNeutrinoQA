...
echo '[INFO] SHINE command:' runModuleSeq.sh -i $DIRECTORY/$INPUT_FILENAME -o $run$chunkNumber -b bootstrap.xml -k $GLOBAL_KEY -v pp -m on
...
make
source runModuleSeq.sh -i $DIRECTORY/$INPUT_FILENAME -o $BASE_NAME -b bootstrap.xml -k $GLOBAL_KEY -v pp -m on
