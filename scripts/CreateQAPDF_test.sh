...
EOSDropDirectory='/afs/cern.ch/user/n/na61qa/workDirectory/2024NeutrinoRun/NativeReconstruction'
pdfMakerDirectory='/afs/cern.ch/user/n/na61qa/workDirectory/2024NeutrinoRun/pdfMaker'

cd $EOSDropDirectory

#List of QA rootfile template names.
qaFiles=("BPDQA" "MRPCQA" "VDQA" "TPCClusterQA" "TPCVertexQA" "TDAQQA" "GRCClusterQA" "vDrift" "TOFFQA" "DetectorCorrelationQA")
#qaFiles=("TDAQQA")
...
do
   ...
   # hadd -k -f $EOSDropDirectory'/'$qaName.root `cat haddList$qaName.txt`
    cp  $EOSDropDirectory'/'$matchString $EOSDropDirectory'/'$qaName.root 
